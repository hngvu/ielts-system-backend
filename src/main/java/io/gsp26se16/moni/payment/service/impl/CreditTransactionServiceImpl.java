...
public class CreditTransactionServiceImpl implements CreditTransactionService {
    ...
    @Override
    public List<CreditTransactionResponse> searchCreditTransactions(Integer userId, String paymentType, LocalDateTime startDate, LocalDateTime endDate) {
        Specification<CreditTransaction> spec = (root, query, criteriaBuilder) -> {
            ...
            if (paymentType != null) {
                try {
                    ...
                    predicate = criteriaBuilder.and(predicate, 
                        criteriaBuilder.equal(root.get("paymentType"), type));
    ...
    public CreditTransactionResponse getCreditTransactionDetail(Integer id) {
        CreditTransaction creditTransaction = creditTransactionRepository.findById(id)
    ...
    private CreditTransactionResponse mapToCreditTransactionResponse(CreditTransaction creditTransaction) {
        return CreditTransactionResponse.builder()
                ...
                .userId(creditTransaction.getUser() != null ? creditTransaction.getUser().getId() : null)
                .serviceId(creditTransaction.getServicePricing() != null ? creditTransaction.getServicePricing().getId() : null)
                .paymentId(creditTransaction.getPayment() != null ? creditTransaction.getPayment().getId() : null)
                .build();
    }
}

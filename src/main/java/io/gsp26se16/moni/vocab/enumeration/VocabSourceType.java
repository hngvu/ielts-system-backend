package io.gsp26se16.moni.vocab.enumeration;

public enum VocabSourceType {
    MANUAL, // User manually added
    READING_PASSAGE, // Saved from reading passage
    DICTIONARY_LOOKUP, // Saved from dictionary lookup
    LISTENING_PASSAGE, // Saved from listening exercise
    WRITING_EXERCISE, // Saved from writing exercise
    SPEAKING_EXERCISE, // Saved from speaking exercise
}
